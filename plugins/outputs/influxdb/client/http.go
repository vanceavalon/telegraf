...
func (c *httpClient) Write(b []byte) (int, error) {
	req := c.makeWriteRequest(len(b), c.writeURL, c.config.Gzip)
	req.SetBody(b)
...
func (c *httpClient) WriteWithParams(b []byte, wp WriteParams) (int, error) {
	req := c.makeWriteRequest(len(b), []byte(writeURL(c.url, wp)), c.config.Gzip)
	req.SetBody(b)
...
func (c *httpClient) WriteStream(r io.Reader, size int) (int, error) {
	req := c.makeWriteRequest(size, c.writeURL, c.config.Gzip)
...
func (c *httpClient) WriteStreamWithParams(
	...
	req := c.makeWriteRequest(size, []byte(writeURL(c.url, wp)), c.config.Gzip)
	req.SetBodyStream(r, size)
...
func (c *httpClient) doRequest(
	...
	if code != expectedCode || response.Error() != nil {
		err = fmt.Errorf("Response Error: Status Code [%d], expected [%d], [%s]",
